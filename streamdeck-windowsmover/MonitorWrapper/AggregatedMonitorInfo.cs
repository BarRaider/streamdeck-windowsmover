...
namespace BarRaider.WindowsMover.MonitorWrapper
{
    ...
    class AggregatedMonitorInfo
    {
        ...
        private static string MonitorDevicePath(LUID adapterId, uint targetId)
        {
            DISPLAYCONFIG_TARGET_DEVICE_NAME deviceName = new DISPLAYCONFIG_TARGET_DEVICE_NAME();
            deviceName.header.size = (uint)Marshal.SizeOf(typeof(DISPLAYCONFIG_TARGET_DEVICE_NAME));
            deviceName.header.adapterId = adapterId;
        ...
        public static List<MonitorInfo> GetAggregatedMonitorInfo()
        {
            List<MonitorInfo> aggregatedMonitorInfo = new List<MonitorInfo>(); ; 
            var wmiMonitors = GetWMIDisplayInfo();
            var queryMonitors = GetQueryDisplayMonitorInfo();

            foreach (var monitor in queryMonitors)
            {
                string instanceName = String.Join(@"\", monitor.DevicePath.Replace(@"\\?\", "").Replace("#", @"\").Split('\\').Take(3));
                var wmiMonitor = wmiMonitors.FirstOrDefault(m => m.InstanceName.Contains(instanceName));
                if (wmiMonitor == null)
                {
                    Logger.Instance.LogMessage(TracingLevel.WARN, $"Unable to aggregate monitor info. {monitor}");
                    continue;
                }
                if (wmiMonitor.UserFriendlyName != monitor.UserFriendlyName)
                {
                    Logger.Instance.LogMessage(TracingLevel.WARN, $"Unable to aggregate monitor info from UserFriendlyName: {monitor.UserFriendlyName}\n{monitor}");
                    continue;
                }
                aggregatedMonitorInfo.Add(new MonitorInfo(wmiMonitor, monitor));
            }
            Logger.Instance.LogMessage(TracingLevel.INFO, $"GetAggregatedMonitorInfo returned {aggregatedMonitorInfo.Count} aggregated monitors");
            return aggregatedMonitorInfo;
        }
        ...
        private static List<WMIMonitorInfo> GetWMIDisplayInfo()
        {
            ...
            try
            {
                ...
                ManagementObjectCollection wmiMonitors = searcher.Get();

                using (var e = wmiMonitors.GetEnumerator())
                {
                    while (e.MoveNext())
                    {
                        try
                        {
                            ...
                            ManagementBaseObject obj = e.Current as ManagementBaseObject;
                            bool isActive;
                            string instanceName;
                        ...
                        {
                            Logger.Instance.LogMessage(TracingLevel.ERROR, $"GetWMIDisplayInfo Loop Exception {ex}");
                            continue;
                        }
                    }
                }
        ...
        private static string GetStringFromUInt16Array(UInt16[] arr)
        {
            ...
            int length = arr.FirstIndexMatch(o => o == 0);
            if (length < 0)
            {
                length = arr.Length;
            }
            return new string(Array.ConvertAll(arr.Take(length).ToArray(), x => (char)(x)));
        }
